...
merge_after_test = os.environ.get("MERGE_AFTER_TEST", "false").lower() == "true"
...
if merge_after_test and pr and repo:
    merge_step = (
        f'  GH_TOKEN="$GH_DISPATCH_TOKEN" gh pr merge {pr} \\\n'
        f'    --repo strata-reports-ai/{repo} --squash --delete-branch 2>&1 \\\n'
        f'    && echo "Merged PR #{pr} to main after successful tests." \\\n'
        f'    || echo "Warning: merge failed — may already be merged or have conflicts"'
    )
else:
    merge_step = "  # PR already merged or merge not requested — no action needed"
...
prompt = prompt.replace("{{MERGE_STEP}}", merge_step)

with open("/tmp/test-prompt.txt", "w") as f:
    f.write(prompt)

print(f"Test prompt built ({len(prompt)} chars, PR(s) {pr_numbers}, repo {repo}, merge_after_test={merge_after_test})")
