...
# Guard 2b: review passed — PR is still open (waiting for deploy+test, or needs rebase)
...
if [ "${PASSED_COMMENT:-0}" -gt "0" ]; then
  ...
  # If already in-test, deploy was already triggered — don't re-dispatch
  if [ -n "$ISSUE_N" ]; then
    ISSUE_LABELS=$(GH_TOKEN="$GH_DISPATCH_TOKEN" gh issue view "$ISSUE_N" \
      --repo strata-reports-ai/orchestrator-strata-reports \
      --json labels --jq '[.labels[].name]' 2>/dev/null || echo "[]")
    if echo "$ISSUE_LABELS" | grep -q '"in-test"'; then
      echo "PR #${PR_NUMBER} passed review and is already in-test — waiting for test results"
      exit 0
    fi
  fi
  ...
  if [ "$MERGEABLE" == "CONFLICTING" ]; then
    echo "PR #${PR_NUMBER} passed review but has conflicts — dispatching rebase agent"
    if [ -n "$ISSUE_N" ]; then
      GH_TOKEN="$GH_DISPATCH_TOKEN" gh issue edit "$ISSUE_N" \
        --repo strata-reports-ai/orchestrator-strata-reports \
...
Branch ${HEAD_BRANCH} needs to be rebased onto main before deployment.
...
4. Then trigger deploy and transition the orchestrator issue:
   ISSUE_N_R=\$(gh pr view ${PR_NUMBER} --repo ${PR_REPO} --json body --jq '.body' | grep -oP 'orchestrator-strata-reports#\K[0-9]+' | head -1 || true)
   if [ -n \"\$ISSUE_N_R\" ]; then
     GH_TOKEN=\"\$GH_DISPATCH_TOKEN\" gh issue edit \"\$ISSUE_N_R\" --repo strata-reports-ai/orchestrator-strata-reports --remove-label in-progress --add-label in-test 2>/dev/null || true
   fi
   jq -n --arg pr \"${PR_NUMBER}\" --arg issue \"\${ISSUE_N_R:-}\" --arg ref \"${HEAD_BRANCH}\" \
     '{\"ref\":\"main\",\"inputs\":{\"ref\":\$ref,\"pr_number\":\$pr,\"orchestrator_issue\":\$issue}}' | \
   GH_TOKEN=\"\$GH_DISPATCH_TOKEN\" gh api \
     repos/${PR_REPO}/actions/workflows/deploy-api.yml/dispatches \
     --method POST --input -"
  ...
  # MERGEABLE — trigger deploy (merge will happen after tests pass)
  echo "PR #${PR_NUMBER} passed review and is MERGEABLE — triggering deploy"
  if [ -n "$ISSUE_N" ]; then
    GH_TOKEN="$GH_DISPATCH_TOKEN" gh issue edit "$ISSUE_N" \
      --repo strata-reports-ai/orchestrator-strata-reports \
...
STEP 4 — If NO major or moderate issues found, run ALL of these commands in order:
  ...
  gh pr comment ${PR_NUMBER} --repo ${PR_REPO} --body "✅ **Code review passed.** No major or moderate issues found. Deploying to dev for testing."
